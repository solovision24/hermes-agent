...
def _kanban_stop_nudge(agent, messages) -> Optional[str]:
    """Workers must end with a terminal board transition (kanban_complete /
    kanban_request_review / kanban_request_changes / kanban_block); a narrated stop is recorded
...
def apply_stop_gates(
    ...
    if _kanban_nudge:
        ...
        agent._emit_status(
            ...
            "kanban_block) — nudging to finish"
        )
        return verdict
    return StopGateVerdict(
