...
KANBAN_GUIDANCE = (
    ...
    "4. **Recover autonomously by default.** Classify evidence, use bounded "
    "retry/backoff, repair capability preflight, reassign to a qualified "
    "profile, try configured provider/tool fallbacks, request specialist "
    "arbitration, then choose a safe reversible default. Machine, provider, "
    "repository, CI, review, and deployment failures remain agent-owned after "
    "retry exhaustion. Use `kanban_block(kind='needs_input'|'capability', "
    "human_gate=...)` only for an irreducible identity/OAuth consent, binding "
    "legal/customer authorization, or financial purchase decision. The gate "
    "must include evidence for every autonomous stage, the smallest exact ask, "
    "and a proposed safe default. Use `dependency` for peer output and "
    "`transient` for flaky failures.\n"
    "5. **Finish with the native review model encoded by the task graph.** "
    "Always include the structured handoff (`summary`, `metadata`) on the "
    "lifecycle transition itself; never put secrets, tokens, or raw PII in "
    "these durable fields. If `kanban_show()` lists child IDs, inspect those "
    "cards with `kanban_show(task_id=...)` before choosing the terminal action. "
    "When a pre-created review, QA, or release child depends on your task, call "
    "`kanban_complete`: completion releases that downstream lane. Never "
    "sticky-block that parent for `review-required` or request same-card review "
    "as well. Otherwise, implementation lanes (DEV/Quill/Chip/Forge) that open "
    "a pull request must call `kanban_submit_review(reviewer=orion, summary=..., "
    "metadata=...)` with pr_url/repo/number/head_sha/verification_evidence as "
    "the terminal handoff, then stop mutating the card. The reviewer approves "
    "with `kanban_complete`, returns actionable rework with "
    "`kanban_review_changes`, or uses `kanban_block` only for a genuine R3 "
    "human escalation. Review is not a block, so repeated review cycles do not "
    "trip unblock-loop detection.\n"
    ...
    "- Do not complete a task you didn't actually finish. Route a dependency "
    "or transient retry, or use the structured irreducible human gate.\n"
    ...
    "`kanban_comment` the context, then use the structured irreducible "
    "`human_gate` only when the autonomous recovery ladder is exhausted.\n"
