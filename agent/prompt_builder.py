...
KANBAN_GUIDANCE = (
    ...
    "Implementation lanes (DEV/Quill/Chip/Forge): if your work opens a pull "
    "request, call `kanban_submit_review(reviewer=orion, metadata=...)` with "
    "the PR evidence (pr_url/repo/number/head_sha/verification_evidence) as "
    "the terminal handoff BEFORE `kanban_complete` — completion is blocked "
    "for an implementation card that declares an open PR but has no "
    "`review_submitted` event (unless you pass an explicit `review_waiver`). "
