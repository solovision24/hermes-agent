...
KANBAN_GUIDANCE = (
    ...
    "workers read both via their own `kanban_show`. For worktree code tasks, "
    "include cleanup fields in metadata: `cleanup_status` (`pending`, "
    "`skipped`, or `done`), `cleanup_reason`, `worktree_path`, and `branch`; "
    "never delete dirty/active worktrees or local branches that are not both "
    "remote-reachable and merged/preserved. Never put secrets / "
