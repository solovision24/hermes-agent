...
def _set_worker_pid(
    conn: sqlite3.Connection, task_id: str, pid: int, *, log_offset: Optional[int] = None,
) -> None:
    ...
    ``log_offset`` (optional) is the byte size of ``<task>.log`` at spawn time — crash diagnosis reads
    only bytes after that offset so a silent re-run cannot inherit a prior attempt's log tail.
    """
