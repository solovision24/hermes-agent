...
def run_doctor(args):
    ...
    if config_path.exists():
        ...
        # reason where operators already look. Rung issues ride inside
        # validate_config_structure() — surface them here only when the
        # structure pass produced nothing, so the section isn't duplicated.
        try:
            from hermes_cli.config import validate_config_structure as _vcs_for_rungs

            if not _vcs_for_rungs():
