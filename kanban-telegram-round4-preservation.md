...
- Working candidate SHA: `842a21cc4a` (local combined candidate)
- Working candidate tree: `7de9dbcafff491f3b86d9953b396a4d3fbfcd7c0`
...
- `tests/gateway/test_kanban_notifier.py`: the real watcher matrix records every Halo adapter text and artifact method before raising; missing-token, wrong-identity, transport retry, response-thread rejection, review delivery, and multipart retry cases assert zero Halo attempts after each tick, including the dedup tick. Done→reopen status routing and archive cleanup assertions remain covered.
- `tests/gateway/test_kanban_changes_requested_notifier.py` and `tests/gateway/test_kanban_notifier_wake_only_ordering.py`: legacy preservation fixtures now exercise the dedicated sender at the network boundary, retaining canonical destination/no-topic and wake ordering assertions.
...
- Focused real watcher preservation suite: 44 passed (including in-process `/tmp/kanban-integrate` imports).
