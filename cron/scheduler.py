...
def _upsert_incident_for_failure(
    ...
    as if none existed. ``failure_type`` overrides the store's keyword classification when the
    caller already knows the fault class (e.g. a failed DELIVERY rather than a failed run)."""
    try:
...
def _record_delivery_failure_incident(job: dict, delivery_error: Optional[str]) -> Optional[str]:
    ...
    withheld, incident_id = _upsert_incident_for_failure(
