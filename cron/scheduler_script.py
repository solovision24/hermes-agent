...
def _run_job_script(
    ...
    try:
        ...
        # The firing profile's secret scope rides the caller-extras channel
        # (_profile_scope_env): a gateway-claimed fire installs the scope as a ContextVar and,
        # under multiplex, never in os.environ, so without this overlay every env-reading
        # no_agent script failed on the gateway path while ticker fires kept working. The §2.3
        # scrub applies to both channels, so Tier-1/Tier-2 credentials stay stripped.
        env = build_subprocess_env(strip_launch_profile=True, extra=_profile_scope_env())
