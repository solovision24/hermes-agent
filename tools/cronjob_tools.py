...
def cronjob(
    ...
    try:
        ...
        job = get_job(job_id)
        if not job:
            return json.dumps({'success': False, 'error': f'Job with ID "{job_id}" not found.'}, indent=2)
...
def schedule_cronjob(prompt: str, schedule: str, name: Optional[str] = None, repeat: Optional[int] = None,
                    deliver: Optional[str] = None, model: Optional[str] = None, provider: Optional[str] = None,
                    base_url: Optional[str] = None, agent_id: Optional[str] = None, task_id: str = None) -> str:
    return cronjob(action='create', prompt=prompt, schedule=schedule, name=name, repeat=repeat, deliver=deliver,
                   model=model, provider=provider, base_url=base_url, agent_id=agent_id, task_id=task_id)
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.''',
    'parameters': {
        ...
        'properties': {
            ...
            'agent_id': {'type': 'string', 'description': "Optional agent profile ID to run the job under (e.g. 'nova', 'orion', 'canon'). The job will run with that agent's config.yaml, SOUL.md, skills, and memories."},
            ...
            'skills': {'type': 'array', 'items': {'type': 'string'}, 'description': 'Optional ordered list of skills to load before executing the cron prompt.'},
