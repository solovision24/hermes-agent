...
def test_webhook_first_native_submission_reconciles_one_card(board):
    with board as conn:
        ...
        assert kb.get_task(conn, webhook_id).status == "review"
        assert kb.get_task(conn, implementation_id).status == "archived"
