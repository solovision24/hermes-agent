...
- For the CLI, use `hermes kanban submit-review <task> <summary words...>`; the default reviewer is `orion` when that profile is installed and otherwise the guaranteed `default` profile. To select another reviewer, use the unambiguous `--reviewer <profile>` flag; the first summary word is never interpreted as a reviewer.
